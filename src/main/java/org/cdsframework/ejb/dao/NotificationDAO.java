 ...
 */
@Stateless
public class NotificationDAO extends BaseDAO<NotificationDTO> {

    @Override
    protected void initialize() throws MtsException {


        this.registerDML(NotificationDTO.ByNotificationStatus.class, new QueryCallback(getDtoTableName()) {

            @Override
            protected String getQueryDML(BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) {
                return "select * from notification where status = :status";
            }
        }, false);
