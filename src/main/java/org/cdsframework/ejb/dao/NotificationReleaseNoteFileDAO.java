 ...
 */
@Stateless
public class NotificationReleaseNoteFileDAO extends BaseDAO<NotificationReleaseNoteFileDTO> {

    @Override
    protected void initialize() throws MtsException {

        // General search query
        this.registerDML(NotificationReleaseNoteFileDTO.ByteArrayByFileId.class, new QueryCallback(getDtoTableName()) {
            @Override
            public String getQueryDML(BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) {
                String sql = "select source_file from notification_release_note_file where file_id = :file_id";
                logger.info("NotificationReleaseNoteFileDTO.ByteArrayByFileId - ", sql);
                return sql;
            }
            ...
            protected void getCallbackNamedParameters(MapSqlParameterSource namedParameters, BaseDTO baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO) throws MtsException {
                if (baseDTO instanceof NotificationReleaseNoteFileDTO) {
                    NotificationReleaseNoteFileDTO notificationReleaseNoteFileDTO = (NotificationReleaseNoteFileDTO) baseDTO;
                    namedParameters.addValue("file_id", notificationReleaseNoteFileDTO.getFileId());
                }
            }
        }, false);
    }
