...
public abstract class BaseBO<T extends BaseDTO> {
    ...
    private Map<Class, Configuration<? extends BaseDTO>> childConfigurations = new HashMap<>();
    ...
    private Map<Class, Comparator> parentChildComparatorMap = new HashMap<>();
    @EJB
    ...
    protected void initializeMain() throws MtsException {
        ...
        try {
            ...
            if (configuration == null) {
                configuration = new Configuration<>(dtoClass, this, debugBO);
            }
    ...
    private <C extends BaseDTO> void registerChildConfiguration(Class<C> childDto, Class queryClass) {
        ...
        try {
            ...
            if (dtoClass == childDto) {
                ...
                childConfiguration = new Configuration<>((BaseBO<C>) this, childDto, queryClass, debugBO);
            } else {
                logger.debug(METHODNAME, "The Parent is not a child, get access to the child and register it");
                childConfiguration = new Configuration<>(childDto, queryClass, debugBO);
            }
...
//    }
    ...
    private HashMap<Object, T> getBaseDTOHashMap(List<T> baseDTOs) throws MtsException {
        HashMap<Object, T> baseDTOMap = new HashMap<>();
    ...
    public byte[] getReport(T baseDTO, SessionDTO sessionDTO, PropertyBagDTO propertyBagDTO)
        ...
        Boolean useDBPrefix = ObjectUtils.objectToBoolean(getPropertyMGRLocal().get("JASPER_REPORTS_USE_DB_PREFIX"), false);
        logger.debug(METHODNAME, "use DB Prefix ", useDBPrefix);
    ...
    public static class Configuration<S extends BaseDTO> {
        ...
        private final Class<S> dtoClass;
        ...
        public List<Class> getChildQueryClassDeleteOrder() {
            if (childQueryClassDeleteOrder == null) {
                childQueryClassDeleteOrder = new LinkedList<>();
                List<ParentChildRelationship> parentChildRelationships = new ArrayList<>(DTOUtils.getParentChildRelationshipMapByDTO(dtoClass).values());
        ...
        public List<Class> getChildQueryClassAddUpdateOrder() {
            if (childQueryClassAddUpdateOrder == null) {
                childQueryClassAddUpdateOrder = new LinkedList<>();
                List<ParentChildRelationship> parentChildRelationships = new ArrayList<>(DTOUtils.getParentChildRelationshipMapByDTO(dtoClass).values());
